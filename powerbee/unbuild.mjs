...
if (!password) {
  console.error('Password required:  node powerbee/unbuild.mjs "<password>"');
  process.exit(1);
}
...
const field = (name) => {
  ...
  if (!match) {
    console.error(`Could not find "${name}" in powerbee/index.html — is it a built page?`);
    process.exit(1);
  }
...
if (!iterations) {
  console.error('Could not find "iterations" in powerbee/index.html — is it a built page?');
  process.exit(1);
}
