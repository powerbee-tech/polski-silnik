 ...
 *
 *   node deck/unbuild.mjs "<password>"
 *   PAGE_PASSWORD="<password>" node deck/unbuild.mjs
 *
...
if (!password) {
  console.error('Password required:  node deck/unbuild.mjs "<password>"');
  process.exit(1);
}
...
const field = (name) => {
  ...
  if (!match) {
    console.error(`Could not find "${name}" in deck/index.html — is it a built page?`);
    process.exit(1);
  }
...
if (!iterations) {
  console.error('Could not find "iterations" in deck/index.html — is it a built page?');
  process.exit(1);
}
...
try {
  ...
  console.error('Wrong password — could not decrypt deck/index.html.');
  process.exit(1);
}
...
console.log(`deck/content/deck.html restored — ${size(plaintext.byteLength)}. It is gitignored; keep it that way.`);
